 ...
 *                                                                        *
 *  Release Date    : Thu Mar 25 21:41:16 PDT 2021                        *
...
public:
  ...
  // Thread for handling reads to system memory
  void memory_read() {
  ...
  // Thread for handling writes to system memory
  void memory_write() {
  ...
  void stim() {
    ...
    biasAlg1x1<DTYPE,MAX_HEIGHT,MAX_WIDTH,OUT_FMAP,MEM_SIZE> bias1x1;
...
#endif
    {
      ...
      if (layer < 8) {
        ...
        wait(100);
        biasAlg(bias_mem,output_feature_maps,fmap_height,fmap_width,woffset,woffset,bias_offset); // Add bias in place
        bias_offset += out_fmaps[layer];
        wt_offset += input_feature_maps*output_feature_maps*KSIZESQ; // move to next set of weights
        weight_offset.write(wt_offset);
        leakyReLuAlg(output_feature_maps,fmap_height,fmap_width,woffset,woffset); // WReLu in place

        if (layer < 6) { // Max pool on first 6 layers
          if (layer == 5) { stride = 1; }
          maxPoolAlg(output_feature_maps,fmap_height,fmap_width,woffset,roffset,stride); // Write back to orignal fmap location
        ...
        bias1x1.run(mem,bias_mem,mem,output_feature_maps,fmap_height,fmap_width,woffset,woffset,bias_offset); // Add bias in place
      }
      wait(100);
    }
    sec = time(NULL) - sec;
